...
#		define RENDER_DEBUG(subSectors, message, ...)	qz::Logger::get()->log(qz::LogVerbosity::DEBUG, __FILE__, __LINE__, std::string("[RENDERING]").append(subSectors), message, __VA_ARGS__)
#	else
...
#ifdef QZ_DEBUG
#	define GLCheck(x) x; qz::gfx::gl::checkError(__FILE__, __LINE__);
#else
#	define GLCheck(x) x;
#endif
...
#ifndef QZ_OS_WINDOWS  // Doesn't exist on Windows.
