...
Uniform::Uniform () : type (UniformType::INVALID), location (-1), name (nullptr)
{
}
...
void GLRenderDevice::showShaderDebugUI ()
{
	...
	if (ImGui::CollapsingHeader ("Uniforms", ImGuiTreeNodeFlags_DefaultOpen))
	{
		for (std::size_t i = 0; i < m_uniformHandleAllocator.size (); i++)
		{
			...
			// Maybe should use setUniformValue here? (instead of manually
			// updating GL) allthough a) this is not permenant (and really
			// should move when the meta query API is implemented. b) ImGui will
			// modify the uniform.userdata values in place so this does the same
			// job as setUniformValue anyway.
			switch (uniform.type)
			{
			case UniformType::COLOR3:
				if (ImGui::ColorPicker3 (uniform.name, uniform.userdata.vec3))
				{
					glUniform3f (uniform.location, uniform.userdata.vec3[0],
					             uniform.userdata.vec3[1],
					             uniform.userdata.vec3[2]);
				}
				break;
			case UniformType::MAT4:
				if (ImGui::InputMatrix4x4 (uniform.name,
				                           &uniform.userdata.mat4))
				{
					glUniformMatrix4fv (uniform.location, 1, GL_FALSE,
					                    uniform.userdata.mat4.elements);
				}
				break;
			case UniformType::VEC2:
				if (ImGui::InputFloat2 (uniform.name, uniform.userdata.vec2))
				{
					glUniform2f (uniform.location, uniform.userdata.vec3[0],
					             uniform.userdata.vec3[1]);
				}
				break;
...
ShaderPipelineHandle GLRenderDevice::createShaderPipelineFromSource (
...
ShaderPipelineHandle GLRenderDevice::createShaderPipelineFromFile (
    const std::string& filepath, const InputLayout& inputLayout)
{
	...
	{
		LFATAL ("Shader source code error at %d : %d filename %s Not found!",
		        filepath[0]);
		assert (false);
		return ShaderPipelineHandle ();
	}

	return this->createShaderPipelineFromSource (dirname, sourcecode,
	                                             inputLayout);
}

void GLRenderDevice::setBufferData (VertexBufferHandle buffer, float* data,
...
void GLRenderDevice::draw (std::size_t first, std::size_t count)
{
	...
	for (const VertexElement& vertexElement : inputLayout.elements)
	{
		...
		GLuint attributeIndex =
		    static_cast<GLuint> (vertexElement.attributeIndex);
		glVertexAttribPointer (attributeIndex, vertexElement.type.numComponents,
		                       ralTypeToGLType (vertexElement.type.type),
		                       vertexElement.normalized ? GL_TRUE : GL_FALSE,
		                       stream.stride,
		                       (void*) (stream.offset + vertexElement.offset));
...
void GLRenderDevice::setVertexBufferStream (VertexBufferHandle buffer,
...
UniformHandle GLRenderDevice::createUniform (ShaderPipelineHandle shaderHandle,
                                             const char* name, UniformType type)
{
...
void GLRenderDevice::setUniformValue (UniformHandle uniform, const void* value,
                                      int num)
{
...
TextureHandle GLRenderDevice::createTexture (unsigned char* pixelData,
                                             std::size_t    width,
                                             std::size_t    height)
{
